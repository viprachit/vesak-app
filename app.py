...
def save_invoice_to_gsheet(data_dict, sheet_obj):
    ...
    try:
        # Convert dictionary to list in specific order
        # Columns: Serial No., Invoice Number, Date, Generated At, Customer Name, Age, Gender, Location, Address, Mobile, 
        # Plan, Shift, Recurring Service, Period, Visits, Amount, Notes / Remarks, Generated By, 
        # Amount Paid, Details, Service Started, Service Ended
...
def mark_service_ended(sheet_obj, invoice_number):
    if sheet_obj is None: return False
    try:
        # Find cell with invoice number
        cell = sheet_obj.find(str(invoice_number))
        if cell:
            # Service Ended is Column 22 (V)
            # Row, Col
            ...
            try:
                ...
                if not selected_label:
                    ...
                    with col1:
                        ...
                        if is_duplicate and not force_new:
                            ...
                            if force_new and is_duplicate:
                                # Generate next number if forcing new on same day
                                default_inv_num = get_next_invoice_number_gsheet(inv_date, df_history)
                    ...
                    btn_label = "Generate Duplicate Copy (PDF Only)" if (is_duplicate and not force_new and force_print) else "Generate & Save Invoice"
                    
                    if st.button(btn_label):
                        
                        if is_duplicate and not force_new and not force_print:
                            st.error("❌ Invoice already exists! Enable 'Force New' or 'Print Duplicate'.")
                            st.stop()
                        ...
                        if not force_print:
                            ...
                            invoice_record = {
                                ...
                                "Amount": float(unit_rate_val), # CHANGE: Saving Unit Rate (Rate Agreed)
                                "Notes / Remarks": str(final_notes),  
                                "Generated By": str(generated_by),
                                "Amount Paid": float(total_billed_amount), # CHANGE: Saving Total Paid
            ...
            if 'Service Ended' not in df_hist.columns:
                ...
                if not active_services.empty:
                    ...
                    if selected_service_disp:
                        ...
                        st.warning(f"Are you sure you want to end service for **{name_to_end}** (Invoice: {inv_to_end})?")
                        
                        if st.button("Mark Service as ENDED 🛑"):
                            success, time_ended = mark_service_ended(sheet_obj, inv_to_end)
                            if success:
