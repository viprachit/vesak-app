...
SHEET_HEADERS = [
    ...
    "Nurse Name", "Nurse Name (Extra)", "Nurse Note"  # <--- NEW COLUMNS ADDED
]
...
def format_worksheet_header(ws):
    try:
        ...
        # Updated Widths for 34 Columns
        widths = [ 
            50, 50, 100, 120, 100, 120, 180, 50, 60, 100, 200, 100, 120, 100, 80, 80, 60, 80, 
            150, 100, 100, 200, 100, 100, 100, 120, 80, 100, 100, 80, 100,
            150, 150, 200 # <-- Widths for Nurse Name, Extra, Note
        ]
...
def save_invoice_to_gsheet(data_dict, sheet_obj):
    ...
    try:
        ...
        row_values = [
            ...
            "", "", "" # <--- Placeholders for AF (Nurse), AG (Extra), AH (Note)
        ]
...
def run_system_health_check(drive_service, sheet_obj_25_id, agreements_id, invoices_id_config):
    ...
    try:
        ...
        years_to_check = [current_yy - 1, current_yy] 
        
        for y_int in years_to_check:
            y_str = str(y_int)
            wb_name = f"Vesak_Invoice_{y_str}"
            if y_int == current_yy: check_month = now.strftime("%b-%y").capitalize() 
