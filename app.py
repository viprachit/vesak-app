...
# --- CRITICAL FIX 1: ID NORMALIZATION ---
...
def render_invoice_ui(df_main, mode="standard"):
    ...
    # --- CRITICAL FIX 1: NORMALIZE IDs (No Decimals) ---
    ...
    with col1:
        ...
        if not conflict_exists:
            ...
            st.info(f"ℹ️ New Invoice: {inv_final}")
        else:
            st.warning(f"⚠️ Active Invoice Found: {inv_final}")
    ...
    with col2:
        ...
        # --- CRITICAL FIX 3: GENERATED BY LOGIC ---
        # "The EDITABLE SECTION should always appear BLANK" -> value=""
        gen_by_input = st.text_input("Generated By:", value="", placeholder="Leave blank for Default", key=f"gen_{mode}")
        # Logic for saving: If Input is Empty -> "Vesak Patient Care"
    ...
    # --- CRITICAL FIX 4: INTERACTIVE EXCLUDED LIST ---
    with sc2: 
        # Use multiselect to allow removing items (clicking X)
        # Default is the full list. Removing an item removes it from 'exc_final'
        exc_final = st.multiselect("Excluded (Click X to remove):", options=exc_list, default=exc_list, key=f"exc_{mode}_{c_ref}")
        # Join back for PDF/Sheet
    ...
    if btn_save:
        ...
        record = {
            "UID": "", # Logic in save function for append, or preserve for update
            ...
            "Amount Paid": total, 
            "Notes / Remarks": notes, 
            "Generated By": gen_by_to_save, # Logic applied here
            "Service Started": datetime.datetime.now().strftime("%Y-%m-%d"), 
            "Service Ended": "",
        ...
        if conflict_exists and chk_overwrite and existing_row_idx:
            ...
            record["UID"] = df_history.iloc[existing_row_idx-2]["UID"] # Preserve UID from history (approx index)
            # Better to not send UID in update dict if we want to be safe, or fetch it exactly.
            # update_invoice_in_gsheet handles columns A-X. 
            # We pass the record. The function writes to the row index found earlier.
