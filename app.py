...
# --- UPDATED: UPDATE IN GSHEET (New Column Structure + Logic) ---
...
if raw_file_obj:
    ...
    if df is not None:
		...
		# Check if history exists and has the key column 'Ref. No.'
		if not df_history_data.empty and 'Ref. No.' in df_history_data.columns:
        	    # Get unique Reference Numbers from the CONFIRMED sheet (df_main)
        	    unique_refs = df_main['Ref. No.'].unique()
        
        	    for ref in unique_refs:
            		# Check history using the Ref. No. strictly
            		is_ended = check_if_service_ended_history(df_history_data, ref)
            
            		if mode == "standard":
                	    # Show if NOT ended (Active or New client)
                	    if not is_ended: valid_ref_nos.append(str(ref))
            		elif mode == "force_new":
                	    # Show ONLY if ended (Inactive client)
                	    if is_ended: valid_ref_nos.append(str(ref))
    		else:
        	    # No history: All clients from Confirmed sheet go to standard mode
        	    if mode == "standard":
             		valid_ref_nos = df_main['Ref. No.'].astype(str).tolist()
    
    		# Filter the main DataFrame (Confirmed Sheet) to show only valid clients based on the list above
